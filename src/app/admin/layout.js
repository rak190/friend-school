...
import { 
  ...
  LogOut, Menu, X, ShieldAlert, Database, ChevronLeft, ChevronRight 
} from 'lucide-react';
...
export default function AdminLayout({ children }) {
  ...
  const navLinks = [
    { name: 'ទិដ្ឋភាពទូទៅ', path: '/admin', icon: <LayoutDashboard className="w-5 h-5" /> },
    { name: 'គ្រប់គ្រងអ្នកប្រើប្រាស់', path: '/admin/users', icon: <Users className="w-5 h-5" /> },
    { name: 'កំណត់ប្រព័ន្ធសាលា', path: '/admin/settings', icon: <Settings className="w-5 h-5" /> },
    { name: 'កំណត់ត្រាសកម្មភាព', path: '/admin/logs', icon: <ShieldAlert className="w-5 h-5" /> },
    { name: 'បម្រុងទុកទិន្នន័យ', path: '/admin/backup', icon: <Database className="w-5 h-5" /> },
  ];
      ...
      <aside className={`${rightMenuOpen ? 'flex' : 'hidden'} lg:flex absolute lg:relative top-[73px] lg:top-0 right-0 w-80 h-[calc(100vh-73px)] lg:h-full bg-white flex-col py-6 px-6 z-40 flex-shrink-0 shadow-2xl lg:shadow-none overflow-y-auto`}>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-700 font-bold border-2 border-white shadow-sm shrink-0">AD</div>
            <div>
              <h4 className="font-bold text-slate-800 text-sm">System Admin</h4>
              <p className="text-xs text-brand-muted">អ្នកគ្រប់គ្រង</p>
            </div>
          </div>
